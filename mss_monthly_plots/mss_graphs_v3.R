...
## download the current year mss data
download.file('http://www.env.gov.bc.ca/wsd/data_searches/snow/asws/data/allmss_current.csv','current_yr_mss.csv')

## download the historic mss data
download.file('http://www.env.gov.bc.ca/wsd/data_searches/snow/asws/data/allmss_archive.csv','all_mss.csv')

## read the just downloaded files into R
hist_mss <- read.csv('all_mss.csv', stringsAsFactors = FALSE)
cur_yr <- read.csv('current_yr_mss.csv', stringsAsFactors = FALSE)
...
#merge the responsibility with the data
#all_mss <- merge(all_mss, responsibility, by.x = "Number", by.y = "Survey.Site", all.x = TRUE)

#for all sites with unknwon responsibilty assignt them unknown
#all_mss$Responsibility[is.na(all_mss$Responsibility)] <- "Unknown"
...
for (i in c(1:length(mss_id))) {
  ...
  logoFile = NA
  if (grepl("BC Hydro", responsibility, ignore.case = T)) {logoFile = "BC_Hydro_Logo.jpg"}
  if (grepl("BC Gov", responsibility, ignore.case = T)) {logoFile = "bcmark_pos.png"}
  if (grepl("Metro Vancouver", responsibility, ignore.case = T)) {logoFile = "MV_logo.png"}
  if (is.na(logoFile)) {logoFile = "bcmark_pos.png"}
...
Plot Generated: ", Sys.Date())) +
  ...
  cowplot::ggsave2(paste0("./Graphs4Upload/", unique(one_stn_data$Number), ".pdf"), p2, scale = 1, width=20, height=15, units="cm", dpi = 150, title = "MSS Snow Report")
  
  
  #ggsave(paste(unique(one_stn_data$Number), ".png", sep = ""), p1, device = "png", path = "C:/Users/jkrogh/Documents/R_Code/MSS_Reports/Graphs",
   #      scale = 1, width = 15, height = 15, units = "cm", dpi = 200, limitsize = TRUE)
  } else {
...
Plot Generated: ", Sys.Date())) +
    ...
    cowplot::ggsave2(paste0("./Graphs4Upload/", unique(one_stn_data$Number), ".pdf"), p4, scale = 1, width=20, height=15, units="cm", dpi = 150, title = "MSS Snow Report")
    
    #ggsave(paste(unique(one_stn_data$Number), ".png", sep = ""), p3, device = "png", path = "C:/Users/jkrogh/Documents/R_Code/MSS_Reports/Graphs",
    #       scale = 1, width = 15, height = 15, units = "cm", dpi = 200, limitsize = TRUE)
  }
...
source("upload_mss_all_data_plots.R")
upload_mss_all_data_plots()
